         ...
         <xsl:when test="@chain">
            ...
            <xsl:for-each select="$elements[position() != last()]">
               ...
               <xsl:variable name="from"
                             select="if ($i = 1) then tokenize($e1, '@') else tokenize($e1, '#')[position() &gt; 1]"/>
               <xsl:variable name="to"
                             select="if ($i = $last) then tokenize($e2, '@') else reverse(tokenize($e2, '#')[position() = (1,2)])"/>
               <message xmlns="http://www.w3.org/1999/XSL/Transform">
               ...
               <xsl:variable name="fromPort" select="if (count($from)=2) then $from[2] else 'output'"/>
               <xsl:variable name="toPort" select="if (count($to[2])=2) then $to[2] else 'input'"/>
